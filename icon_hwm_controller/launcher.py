...
try:
  ...
  from icon_hwm_controller.proto import icon_hwm_controller_config_pb2
except ImportError:
  import ros2_hwm_config_pb2
  import icon_hwm_controller_config_pb2
...
def main():
  ...
  context_name = hw_module_config.context_name if hw_module_config.context_name else hw_module_config.name
  if context_name:
    cmd.append(f'context_name:={context_name}')

  cmd.append(f'drives_realtime_clock:={"true" if hw_module_config.drives_realtime_clock else "false"}')
  cmd.append(f'lock_memory:={"true" if icon_cfg.lock_memory else "false"}')
  ...
  bash_cmd = f"export PYTHONUNBUFFERED=1 && export RCUTILS_LOGGING_BUFFERED_STREAM=0 && source /opt/ros/kilted/setup.bash && source /ament_ws/install/setup.bash && {' '.join(cmd)}"
