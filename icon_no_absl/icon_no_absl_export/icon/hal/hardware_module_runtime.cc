...
// Options configuring thread properties such as name, real-time priority,
// scheduling policy, and CPU affinity.
struct ThreadOptions {
  std::string name;
  bool is_realtime = false;
  int priority = 0;
  int schedule_policy = SCHED_OTHER;
...
RemoteTriggerServer::Prelude MakeThreadPrelude(const ThreadOptions& options) {
  return [options]() -> Status {
    pthread_t thread_handle = pthread_self();
    if (!options.name.empty()) {
      std::string short_name = options.name.substr(0, 15);
    ...
    if (options.is_realtime) {
      int policy = options.schedule_policy != SCHED_OTHER
                       ? options.schedule_policy
                       : SCHED_FIFO;
      int priority = options.priority > 0 ? options.priority : 40;
      sched_param sch;
      sch.sched_priority = priority;
      if (int err = pthread_setschedparam(thread_handle, policy, &sch);
          err != 0) {
        if (err == EPERM) {
          return FormatStatus(StatusCode::kPermissionDenied,
                              "Permission denied setting RT priority");
        }
        return FormatStatus(StatusCode::kInternal,
...
class HardwareModuleRuntime::CallbackHandler final {
 ...
 public:
  ...
  void OnReadStatus() INTRINSIC_CHECK_REALTIME_SAFE {
    // TODO(b/542544362): Re-enable MallocGuard once available externally via
    ...
    if (icon::GetThreadLocalMallocViolations().num_violations > 0) {
      auto message = FormatRealtimeStatus(
  ...
  void OnApplyCommand() INTRINSIC_CHECK_REALTIME_SAFE {
    if (hardware_module_state_code_.load(std::memory_order_acquire) ==
        intrinsic_fbs::StateCode::kMotionDisabling) {
      return;
    } else if (hardware_module_state_code_.load(std::memory_order_acquire) !=
  ...
  void CancelPendingRequests(std::string_view cancel_reason)
    ...
    while (!request_queue_.reader()->Empty()) {
      ...
      INTRINSIC_SHARED_MEMORY_LOG(
          ...
          cancel_reason);
      (void)async_request.SetResponse(
  ...
  void ProcessNextPendingRequest() INTRINSIC_CHECK_REALTIME_SAFE {
    ...
    if (!request_queue_.reader()->Empty()) {
      ...
      if (newest_data.timestamp >= hardware_module_state_update_time_ &&
          newest_data.from ==
              hardware_module_state_code_.load(std::memory_order_acquire)) {
        RealtimeStatus req_status = FormatRealtimeStatus(
...
HardwareModuleRuntime::Create(
  ...
  if (control_period < std::chrono::nanoseconds::zero()) {
    return tl::unexpected(
        FormatStatus(StatusCode::kInvalidArgument,
                     "Control period must be non-negative, got: {:d} ns",
  ...
  auto runtime = std::unique_ptr<HardwareModuleRuntime>(
      new HardwareModuleRuntime(std::move(hardware_module), std::move(registry),
...
Status HardwareModuleRuntime::Connect(
  ...
  INTR_ASSIGN_OR_RETURN_STATUS(
      ...
      RemoteTriggerServer::Create(
          *shared_memory_manager_, "activate", logger_,
  ...
  INTR_ASSIGN_OR_RETURN_STATUS(
      auto prepare_server,
      RemoteTriggerServer::Create(
          *shared_memory_manager_, "prepare", logger_,
...
Status HardwareModuleRuntime::Run(bool is_realtime,
  ...
  if (is_realtime) {
    restart_thread_options.SetRealtimeLowPriorityAndScheduler();
    restart_thread_options.SetAffinity(cpu_affinity);
  }
  INTR_RETURN_STATUS_IF_ERROR(restart_server_->StartAsync(
      logger_, MakeThreadPrelude(restart_thread_options)));
  ...
  // TODO(b/542544362): Re-enable MallocGuard options once available via BCR.
  ...
  auto state_change_prelude = MakeThreadPrelude(state_change_thread_options);
  ...
  state_change_thread_ =
      std::jthread([this, state_change_prelude](std::stop_token stop_token) {
        ...
        while (!stop_token.stop_requested() &&
          ...
          (void)prepare_server_->Query(logger_);
          (void)enable_motion_server_->Query(logger_);
          (void)disable_motion_server_->Query(logger_);
          (void)clear_faults_server_->Query(logger_);
        }
      });

  INTR_RETURN_STATUS_IF_ERROR(
      set_init_failed_on_error(activate_server_->StartAsync(
          logger_, MakeThreadPrelude(activate_thread_options))));
  INTR_RETURN_STATUS_IF_ERROR(
      set_init_failed_on_error(deactivate_server_->StartAsync(
          logger_, MakeThreadPrelude(deactivate_thread_options))));
  INTR_RETURN_STATUS_IF_ERROR(
      set_init_failed_on_error(read_status_server_->StartAsync(
          logger_, MakeThreadPrelude(read_status_thread_options))));
  INTR_RETURN_STATUS_IF_ERROR(
      set_init_failed_on_error(apply_command_server_->StartAsync(
          logger_, MakeThreadPrelude(apply_command_thread_options))));
...
void HardwareModuleRuntime::SetStateTestOnly(intrinsic_fbs::StateCode state,
  ...
  callback_handler_->SetStateDirectly(
      ...
      fault_reason.empty()
          ? RtOkStatus()
          : FormatRealtimeStatus(StatusCode::kUnknown, "{:s}", fault_reason),
      /*force=*/true);
}
