...
class TestHardwareModule : public HardwareModuleInterface {
 public:
  ...
  RealtimeStatus Activate() override {
    if (transition_callback_) {
      auto status = transition_callback_(TransitionRequest::kActivate);
      if (!status.ok()) {
  ...
  RealtimeStatus Deactivate() override {
    if (transition_callback_) {
      auto status = transition_callback_(TransitionRequest::kDeactivate);
      if (!status.ok()) {
        return FormatRealtimeStatus(status.code, "{:s}", status.message);
      }
    }
  ...
  RealtimeStatus Disabled() override {
    if (enabled_callback_) {
      auto s = enabled_callback_(false);
      if (!s.ok()) {
        return FormatRealtimeStatus(s.code, "{:s}", s.message);
      }
    }
  ...
  RealtimeStatus ReadStatus() override {
    if (fault_reason_mutex_.TryLock()) {
      std::string reason = fault_reason_;
  ...
  void SetFaultReason(std::string_view reason) {
    MutexLock lock(&fault_reason_mutex_);
    fault_reason_ = std::string(reason);
  }
...
class RemoteTriggerClient {
 public:
  ...
  static tl::expected<RemoteTriggerClient, Status> Create(
    ...
    std::string req_name = std::string(server_name) + kSemRequestSuffix;
    std::string res_name = std::string(server_name) + kSemResponseSuffix;
  ...
  Status Trigger(std::chrono::milliseconds timeout = std::chrono::seconds(5)) {
    if (auto s = request_futex_.GetValue().Post(); !s.ok()) {
      return ToStatus(s);
    }
    if (auto s = response_futex_.GetValue().WaitFor(timeout); !s.ok()) {
      return ToStatus(s);
    }
    return OkStatus();
  }
 ...
 public:
  void SetUp() override {
    ...
    mutable_hardware_module_state_ =
        const_cast<intrinsic_fbs::HardwareModuleState*>(hardware_module_state_);
  }
  ...
  // Requests EnableMotion and calls ReadStatus() until the RemoteTrigger call
  // finishes.
  Status EnableMotion() {
  ...
  // Requests DisableMotion and calls ReadStatus() until the RemoteTrigger call
  // finishes.
  ...
  // Requests ClearFault and calls ReadStatus() until the RemoteTrigger call
  // finishes.
  Status ClearFaults() {
  ...
  // Transitions the module to kMotionEnabled via Prepare, Activate, and
  // EnableMotion.
  ...
  // Simulates a single realtime cycle by triggering ReadStatus and
  // ApplyCommand.
  Status Tick() {
    INTR_RETURN_STATUS_IF_ERROR(ReadStatus());
    return ApplyCommand();
...
TEST(HardwareModuleRuntimeTest, HardwareModuleRuntimeExposesInitFailure) {
  ...
  auto logger = std::make_unique<log::Logger>(log::Logger::Severity::kDebug,
                                              log::MockSink(log_entries));
  ...
  INTR_ASSERT_OK_AND_ASSIGN(
      auto runtime,
      HardwareModuleRuntime::Create(kModuleName, std::chrono::milliseconds(1),
  ...
  INTR_ASSERT_OK_AND_ASSIGN(auto state, runtime->GetHardwareModuleState());
  ...
  INTR_ASSERT_OK_AND_ASSIGN(
      ...
      GetSegmentNameToFileDescriptorMap(
          ...
          std::chrono::seconds(5), logger.get()));
...
TEST(HardwareModuleRuntimeTest,
  ...
  auto runtime_or = HardwareModuleRuntime::Create(
      kModuleName, std::chrono::milliseconds(1),
...
TEST(HardwareModuleRuntimeTest,
  ...
  EXPECT_THAT(runtime_or,
              StatusIs(StatusCode::kInvalidArgument,
                       HasSubstr("Control period must be non-negative")));
}
...
TEST_F(HardwareModuleRuntimeIpcFixture, Prepare) {
  std::unordered_set<intrinsic_fbs::StateCode> op_states = {
      intrinsic_fbs::StateCode::kDeactivated,
  ...
  auto noop_states = NoopStates(op_states);

  for (const auto& state : noop_states) {
    runtime_->SetStateTestOnly(state, "secret message");
    INTR_EXPECT_OK(Prepare());
    EXPECT_EQ(hardware_module_state_->code(), state);
    EXPECT_EQ(GetMessage(hardware_module_state_), "secret message");
  }
...
TEST_F(HardwareModuleRuntimeIpcFixture, FailingPrepareResultsInFatallyFaulted) {
  ...
  EXPECT_EQ(
      std::string_view(reinterpret_cast<const char*>(state.message()->Data())),
      "Prepare failed");
}
...
TEST_F(HardwareModuleRuntimeIpcFixture, Deactivate) {
  std::unordered_set<intrinsic_fbs::StateCode> noop_states = {
  ...
  auto op_states = OpStates(noop_states);
...
TEST_F(HardwareModuleRuntimeIpcFixture, DisableMotion) {
  auto noop_states = NoopStates({intrinsic_fbs::StateCode::kMotionEnabled});
