...
class INTR_LOCKABLE LockableBinaryFutex {
 public:
  ...
  ~LockableBinaryFutex() {
    ...
    if (futex_.Value() != 1) {
      std::cerr << "Futex was not unlocked before destruction" << std::endl;
      std::abort();
    }
  }
  ...
  RealtimeStatus Unlock() INTR_UNLOCK_FUNCTION() {
    if (!IsHeld()) {
  ...
  void AssertHeld() const INTR_ASSERT_EXCLUSIVE_LOCK() {
    if (!IsHeld()) {
      std::cerr << "LockableBinaryFutex is not held when it should be"
                << std::endl;
      std::abort();
    }
  }
...
class INTR_SCOPED_LOCKABLE BinaryFutexLock {
 public:
  ...
  explicit BinaryFutexLock(LockableBinaryFutex* mutex)
      ...
      : mutex_(mutex) {
    if (mutex_ == nullptr) {
      std::cerr << "Cannot initialize LockableBinaryFutexLock from a nullptr"
                << std::endl;
      std::abort();
    }
    if (!mutex_->Lock().ok()) {
      std::cerr << "LockableBinaryFutexLock failed to lock futex" << std::endl;
      std::abort();
    }
  }
  ...
  ~BinaryFutexLock() INTR_UNLOCK_FUNCTION() {
    if (!mutex_->Unlock().ok()) {
      std::cerr << "LockableBinaryFutexLock failed to unlock futex"
                << std::endl;
      std::abort();
    }
  }
