#ifndef PLATFORM_COMMON_BUFFERS_RT_PROMISE_H_
#define PLATFORM_COMMON_BUFFERS_RT_PROMISE_H_
...
// INTR_ASSIGN_OR_RETURN_STATUS(RealtimePromise<bool> promise,
// rt_job_result.GetPromise());
// INTR_RETURN_STATUS_IF_ERROR(rt_thread.Start(rt_thread_options,
//                           [promise = std::move(promise)]() mutable {
//                             // Do fancy real time stuff that'll set the
//                             value
//                             // of `result`.
//                             // ...
...
// INTR_ASSIGN_OR_RETURN_STATUS(bool job_result,
//   rt_job_result.WaitUntilAndGet(intrinsic::Time::max()));
//
// General notes:
...
//   consume the value of the future. The value can be peeked at multiple
//   times.
// * Calling `Set()` on a promise or `Get()` on a future will make the future
// or
...
//   future or promise can be created with `is_reusable = true`. This will
//   allow the future or promise to be reused multiple times, i.e. `Set()` can
//   be called multiple times as long as the value is consumed by a `Get()`
//   inbetween. Similarly, `Get()` can be called multiple times as long as a
//   new value is provided by a `Set()` inbetween each `Get()`. In order to
//   destroy a reusable future or promise, you might want to assign a
//   `DetachedFuture` or `DetachedPromise` to it respectively.
...
// * A default-constructed `RealtimeFuture` or `RealtimePromise` will own
// their
...
//   promise can be destroyed without waiting for a potentially attached
//   promise or future.
// * For full control over the wait-free destruction, we recommend creating
// the
...
// * The `FuturePromiseContext` provides a `Reset` method to reset the state
// of
//   the context. This method will wait for any attached future or promise to
//   be destroyed first.
...
// RealtimeFuture. It manages the underlying buffer, synchronization
// primitives, and state flags.
template <typename T>
class FuturePromiseContext {
 public:
  explicit FuturePromiseContext(
      const log::Logger* logger INTR_ATTRIBUTE_LIFETIME_BOUND = nullptr)
      : FuturePromiseContext(std::chrono::milliseconds(1000), logger) {}

  FuturePromiseContext(
      std::chrono::milliseconds detach_timeout,
      const log::Logger* logger INTR_ATTRIBUTE_LIFETIME_BOUND = nullptr)
      : logger_(logger),
  ...
  // handles to be detached. This ensures that the FuturePromiseContext
  // outlives any active handles unless they are explicitly detached or their
  ...
  ~FuturePromiseContext() {
    ...
    if (future_attached_.load(std::memory_order_acquire)) {
      ...
      if (!status.ok()) {
        // Fatal log, because the surviving Future will have a dangling pointer
        // to this context after this destructor finishes.
        INTRINSIC_SHARED_MEMORY_LOG_FATAL(
            logger_, "Future was not destroyed: {} {}",
            StatusCodeName(status.code), status.GetMessage());
      }
    }
    if (promise_attached_.load(std::memory_order_acquire)) {
      ...
      if (!status.ok()) {
        ...
        // to this context after this destructor finishes.
        INTRINSIC_SHARED_MEMORY_LOG_FATAL(
            logger_, "Promise was not destroyed: {} {}",
            StatusCodeName(status.code), status.GetMessage());
      }
    }
  };
  ...
  // If `is_reusable` is true, the promise can be reused after setting a
  // value.
  tl::expected<RealtimePromise<T>, RealtimeStatus> GetPromise(
    ...
    if (!promise_attached_.compare_exchange_strong(
      ...
      return tl::unexpected(FormatRealtimeStatus(
          StatusCode::kAlreadyExists, "Promise is already attached."));
    }
    INTR_RETURN_UNEXPECTED_IF_ERROR(get_promise_futex_.Unlock());
  ...
  tl::expected<RealtimeFuture<T>, RealtimeStatus> GetFuture(
      bool is_reusable = false) INTRINSIC_CHECK_REALTIME_SAFE {
    ...
    if (!future_attached_.compare_exchange_strong(
      ...
      return tl::unexpected(FormatRealtimeStatus(
          StatusCode::kAlreadyExists, "Future is already attached."));
    }
    INTR_RETURN_UNEXPECTED_IF_ERROR(get_future_futex_.Unlock());
  ...
  // This operation cancels any pending operations, clears the stored value
  // (if any), and resets the internal state flags. The function will wait for
  // any existing future/promise to be detached first. While the reset is in
  // progress, no new future/promise handles can be created and `Cancel()`
  // will return an error.
  Status Reset(std::chrono::milliseconds timeout) INTRINSIC_NON_REALTIME_ONLY {
    // The locks below are used to prevent any future/promise handle to be
    // created while the reset is in progress.
    BinaryFutexLock get_future_lock(&get_future_futex_);
    BinaryFutexLock get_promise_lock(&get_promise_futex_);
    // The cancel lock prevents any cancel operations from happening while the
    // reset is in progress.
    BinaryFutexLock cancel_lock(&cancel_futex_);

    // Wait for the future and promise to be detached.
    if (future_attached_.load(std::memory_order_acquire)) {
      if (auto status = future_detached_.WaitFor(timeout); !status.ok()) {
        return FormatStatus(status.code,
                            "{}: Future was not detached within timeout",
                            status.GetMessage());
      }
    }
    if (promise_attached_.load(std::memory_order_acquire)) {
      if (auto status = promise_detached_.WaitFor(timeout); !status.ok()) {
        return FormatStatus(status.code,
    ...
    BinaryFutexLock read_lock(&read_value_futex_);
    BinaryFutexLock write_lock(&write_value_futex_);
  ...
  RealtimeStatus Cancel() INTRINSIC_CHECK_REALTIME_SAFE {
    if (!cancel_futex_.TryLock()) {
      return FormatRealtimeStatus(StatusCode::kResourceExhausted,
                                  "Cannot cancel while reset is in progress.");
    }

    // Signal cancellation to the promise and future.
    is_cancelled_.store(true, std::memory_order_release);
    INTR_RETURN_STATUS_IF_ERROR(cancel_futex_.Unlock());
  ...
  tl::expected<bool, RealtimeStatus> HasValue() const
      INTRINSIC_CHECK_REALTIME_SAFE {
    if (IsCancelled()) {
      return tl::unexpected(FormatRealtimeStatus(StatusCode::kCancelled,
  ...
  RealtimeStatus TestOnlyResetLock() INTRINSIC_NON_REALTIME_ONLY
      INTR_EXCLUSIVE_LOCK_FUNCTION(read_value_futex_, write_value_futex_,
                                   cancel_futex_, get_future_futex_,
                                   get_promise_futex_) {
    auto status = get_future_futex_.Lock();
    status =
        intrinsic::OverwriteIfNotInError(status, get_promise_futex_.Lock());
    status = intrinsic::OverwriteIfNotInError(status, read_value_futex_.Lock());
    status =
        intrinsic::OverwriteIfNotInError(status, write_value_futex_.Lock());
    status = intrinsic::OverwriteIfNotInError(status, cancel_futex_.Lock());
    return status;
  }

  // Test only method to release a lock for all the futexes used to test the
  // behavior of the context while a reset is in progress.
  RealtimeStatus TestOnlyResetUnlock() INTRINSIC_CHECK_REALTIME_SAFE
      INTR_UNLOCK_FUNCTION(read_value_futex_, write_value_futex_, cancel_futex_,
                           get_future_futex_, get_promise_futex_) {
    auto status = cancel_futex_.Unlock();
    status =
        intrinsic::OverwriteIfNotInError(status, write_value_futex_.Unlock());
    status =
        intrinsic::OverwriteIfNotInError(status, read_value_futex_.Unlock());
    status =
        intrinsic::OverwriteIfNotInError(status, get_promise_futex_.Unlock());
    status =
        intrinsic::OverwriteIfNotInError(status, get_future_futex_.Unlock());
    return status;
  }
 ...
 private:
  ...
  // destructor.
  RealtimeStatus DetachPromise() INTRINSIC_CHECK_REALTIME_SAFE {
    bool previously_attached =
        promise_attached_.exchange(false, std::memory_order_acq_rel);
    if (previously_attached) {
      INTR_RETURN_STATUS_IF_ERROR(promise_detached_.Post());
      return RtOkStatus();
    }
    return FormatRealtimeStatus(StatusCode::kInternal,
                                "Promise was not attached.");
  }

  // Detaches the future from this context. Called by RealtimeFuture
  // destructor.
  RealtimeStatus DetachFuture() INTRINSIC_CHECK_REALTIME_SAFE {
    bool previously_attached =
        future_attached_.exchange(false, std::memory_order_acq_rel);
    if (previously_attached) {
      INTR_RETURN_STATUS_IF_ERROR(future_detached_.Post());
      return RtOkStatus();
    }
    return FormatRealtimeStatus(StatusCode::kInternal,
                                "Future was not attached.");
  }

  bool ReadTryLock() INTRINSIC_CHECK_REALTIME_SAFE
      INTR_EXCLUSIVE_TRYLOCK_FUNCTION(true, read_value_futex_) {
    return read_value_futex_.TryLock();
  }

  RealtimeStatus ReadUnlock() INTRINSIC_CHECK_REALTIME_SAFE
      INTR_UNLOCK_FUNCTION(read_value_futex_) {
    return read_value_futex_.Unlock();
  }

  bool WriteTryLock() INTRINSIC_CHECK_REALTIME_SAFE
      INTR_EXCLUSIVE_TRYLOCK_FUNCTION(true, write_value_futex_) {
    return write_value_futex_.TryLock();
  }

  RealtimeStatus WriteUnlock() INTRINSIC_CHECK_REALTIME_SAFE
      INTR_UNLOCK_FUNCTION(write_value_futex_) {
  ...
  LockableBinaryFutex read_value_futex_;
  LockableBinaryFutex write_value_futex_;
  LockableBinaryFutex cancel_futex_;
  LockableBinaryFutex get_future_futex_;
  LockableBinaryFutex get_promise_futex_;
  // intrinsic:*:strip_begin
  // NOLINTEND(google3-custom-lockable-without-annotations)
  // intrinsic:*:strip_end

  // Futex to signal when a value has been set or an operation cancelled.
  BinaryFutex has_value_;
  // Futex to signal when the future has been detached.
  BinaryFutex future_detached_;
  // Futex to signal when the promise has been detached.
  BinaryFutex promise_detached_;
  // The timeout to wait for the future and promise to be detached during
  // destruction.
  const std::chrono::milliseconds detach_timeout_;
};
...
class RealtimePromise {
 ...
 public:
  ...
  explicit RealtimePromise(
      const log::Logger* logger INTR_ATTRIBUTE_LIFETIME_BOUND = nullptr)
      INTRINSIC_NON_REALTIME_ONLY : is_reusable_(true) {
    internal_context_ = std::make_unique<Context>(logger);
    context_ = internal_context_.get();
    context_->promise_attached_.store(true, std::memory_order_release);
  }

  RealtimePromise(std::chrono::milliseconds detach_timeout,
                  const log::Logger* logger INTR_ATTRIBUTE_LIFETIME_BOUND =
                      nullptr) INTRINSIC_NON_REALTIME_ONLY
      : is_reusable_(true) {
    internal_context_ = std::make_unique<Context>(detach_timeout, logger);
    context_ = internal_context_.get();
    context_->promise_attached_.store(true, std::memory_order_release);
  }
  ...
  ~RealtimePromise() {
    if (context_) {
      const log::Logger* logger = context_->logger_;
      auto status = Detach();
      if (!status.ok()) {
        INTRINSIC_SHARED_MEMORY_LOG_WARNING(
            logger, "Promise was not detached: {} {}",
            StatusCodeName(status.code), status.GetMessage());
      }
    }
  ...
  // This can be used to create a temporary promise in a real-time thread,
  // i.e. to cause the destruction of the assigned-to promise. Relies on RVO
  // of the compiler to instantiate the promise in-place.
  ...
  RealtimeStatus Set(T value) INTRINSIC_CHECK_REALTIME_SAFE {
    if (!context_) {
      return FormatRealtimeStatus(StatusCode::kFailedPrecondition,
                                  "Promise is not attached.");
    }

    if (!context_->WriteTryLock()) {
      // If the try lock fails, the context is being cancelled.
      return FormatRealtimeStatus(StatusCode::kCancelled,
                                  "The promise is being cancelled.");
    }

    if (context_->IsCancelled()) {
      INTR_RETURN_STATUS_IF_ERROR(context_->WriteUnlock());
      return FormatRealtimeStatus(StatusCode::kCancelled,
    ...
    *element = std::move(value);
    ...
    INTR_RETURN_STATUS_IF_ERROR(context_->WriteUnlock());
    if (!is_reusable_) {
      status = intrinsic::OverwriteIfNotInError(status, Detach());
    }
    return status;
  }
  ...
  // Returns a FailedPreconditionError if the promise is not attached.
  tl::expected<bool, RealtimeStatus> IsCancelled() const
      INTRINSIC_CHECK_REALTIME_SAFE {
    if (!context_) {
      return tl::unexpected(FormatRealtimeStatus(
          StatusCode::kFailedPrecondition, "Promise is not attached."));
    }
    // Call the underlying method to check for cancellation, since this has
    // all the logic to handle a concurrent `Reset` call.
 ...
 protected:
  ...
  RealtimeStatus Detach() INTRINSIC_CHECK_REALTIME_SAFE {
    if (!context_) {
      return FormatRealtimeStatus(StatusCode::kFailedPrecondition,
                                  "Promise is not attached.");
    }
...
class RealtimeFuture {
 ...
 public:
  ...
  RealtimeFuture(std::chrono::milliseconds detach_timeout,
                 const log::Logger* logger INTR_ATTRIBUTE_LIFETIME_BOUND =
                     nullptr) INTRINSIC_NON_REALTIME_ONLY : is_reusable_(true) {
    internal_context_ = std::make_unique<Context>(detach_timeout, logger);
    context_ = internal_context_.get();
    context_->future_attached_.store(true, std::memory_order_release);
  }
  ...
  ~RealtimeFuture() {
    if (context_) {
      ...
      if (!status.ok()) {
        INTRINSIC_SHARED_MEMORY_LOG_WARNING(
            logger, "Future was not detached: {} {}",
            StatusCodeName(status.code), status.GetMessage());
      }
    }
    // The destructor of `internal_context_` will wait for any attached
    // promise to be detached before destroying the context.
    internal_context_ = nullptr;
  }
  ...
  tl::expected<T, RealtimeStatus> Get() INTRINSIC_CHECK_REALTIME_SAFE {
    if (!context_) {
      return tl::unexpected(FormatRealtimeStatus(
          StatusCode::kFailedPrecondition, "Future is not attached."));
    }

    if (!context_->ReadTryLock()) {
      // If the try lock fails, the context is being cancelled.
      return tl::unexpected(FormatRealtimeStatus(StatusCode::kCancelled,
                                                 "Future is being cancelled."));
    }

    if (context_->IsCancelled()) {
      INTR_RETURN_UNEXPECTED_IF_ERROR(context_->ReadUnlock());
      return tl::unexpected(FormatRealtimeStatus(StatusCode::kCancelled,
    ...
    if (reader == nullptr) {
      INTR_RETURN_UNEXPECTED_IF_ERROR(context_->ReadUnlock());
      return tl::unexpected(FormatRealtimeStatus(StatusCode::kInternal,
                                                 "Received nullptr reader."));
    }
    ...
    if (element.has_value()) {
      ...
      // available.
      (void)context_->has_value_.TryWait();
      INTR_RETURN_UNEXPECTED_IF_ERROR(context_->ReadUnlock());
      if (!is_reusable_) {
        INTR_RETURN_UNEXPECTED_IF_ERROR(Detach());
      }
  ...
  tl::expected<T, RealtimeStatus> Peek(CopyFn copy_fn = DefaultCopyFn<T>)
      INTRINSIC_CHECK_REALTIME_SAFE {
    if (!context_) {
      return tl::unexpected(FormatRealtimeStatus(
          StatusCode::kFailedPrecondition, "Future is not attached."));
    ...
    if (front == nullptr) {
      ...
      return tl::unexpected(FormatRealtimeStatus(
          StatusCode::kUnavailable, "Value is not available yet."));
    }
    reader->KeepFront();
  ...
  Status WaitFor(std::chrono::milliseconds duration)
      INTRINSIC_NON_REALTIME_ONLY {
    return WaitUntil(intrinsic::Now() + duration);
  }
  ...
  // Returns an error if the timeout occurs, if the future is cancelled or if
  // the future is not attached.
  Status WaitUntil(Time deadline) INTRINSIC_NON_REALTIME_ONLY {
    if (!context_) {
      return FormatStatus(StatusCode::kFailedPrecondition,
                          "Future is not attached.");
    }
    ...
    auto has_value_result = context_->HasValue();
    if (!has_value_result.has_value()) {
      return ToStatus(has_value_result.error());
    }
    if (*has_value_result) {
      return OkStatus();
    }
    ...
    if (auto status = context_->has_value_.WaitUntil(deadline); !status.ok()) {
      return FormatStatus(status.code, "{}: New value is not available yet.",
                          status.GetMessage());
    }

    // If the buffer is empty after the wait, it must have been a
    // cancellation.
    if (context_->IsCancelled()) {
      return FormatStatus(StatusCode::kCancelled, "Future was cancelled.");
    }
  ...
  tl::expected<T, Status> WaitUntilAndGet(Time deadline)
      INTRINSIC_NON_REALTIME_ONLY {
    INTR_RETURN_UNEXPECTED_IF_ERROR(WaitUntil(deadline));
    INTR_ASSIGN_OR_RETURN_UNEXPECTED(auto value, Get());
  ...
  // the future is cancelled or the future is not attached.
  tl::expected<T, Status> WaitForAndGet(std::chrono::milliseconds duration)
      INTRINSIC_NON_REALTIME_ONLY {
    INTR_RETURN_UNEXPECTED_IF_ERROR(WaitFor(duration));
    auto get_result = Get();
    if (!get_result.has_value()) {
      return tl::unexpected(ToStatus(get_result.error()));
    }
  ...
  template <typename CopyFn = decltype(DefaultCopyFn<T>)>
  tl::expected<T, Status> WaitUntilAndPeek(Time deadline,
                                           CopyFn copy_fn = DefaultCopyFn<T>)
      INTRINSIC_NON_REALTIME_ONLY {
    INTR_RETURN_UNEXPECTED_IF_ERROR(WaitUntil(deadline));
    INTR_ASSIGN_OR_RETURN_UNEXPECTED(auto value, Peek(copy_fn));
    return std::move(value);
  }
  ...
  // is not (trivially) copyable.
  template <typename CopyFn = decltype(DefaultCopyFn<T>)>
  tl::expected<T, Status> WaitForAndPeek(std::chrono::milliseconds duration,
                                         CopyFn copy_fn = DefaultCopyFn<T>)
      INTRINSIC_NON_REALTIME_ONLY {
    INTR_RETURN_UNEXPECTED_IF_ERROR(WaitFor(duration));
    auto peek_result = Peek(copy_fn);
    if (!peek_result.has_value()) {
      return tl::unexpected(ToStatus(peek_result.error()));
    }
    return std::move(*peek_result);
  }
 ...
 protected:
  ...
  // A reusable future won't detach from the context after its `Get()` method
  // is called and thus can be reused again to get a new value.
  bool is_reusable_{false};
};

}  // namespace intrinsic

#endif  // PLATFORM_COMMON_BUFFERS_RT_PROMISE_H_
