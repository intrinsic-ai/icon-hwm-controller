...
class FuturePromiseContextTest : public ::testing::Test {
 public:
  ...
  // needed for std::unique_ptr.
  template <typename V>
  std::function<V(const V&)> GetTestCopyFunction() {
    return [](const V& value) { return value; };
  }

  // Specialization for std::unique_ptr<int>.
  template <>
  std::function<std::unique_ptr<int>(const std::unique_ptr<int>&)>
  GetTestCopyFunction() {
  ...
  log::Logger logger_{log::Logger::Severity::kDebug,
                      log::MockSink(log_entries_, /*log_prefix=*/kLogPrefix)};
};
