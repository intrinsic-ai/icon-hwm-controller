...
//
// // Pop returns std::nullopt if queue was empty.
 ...
 public:
  ...
  class Reader {
   public:
    ...
    // allowed.
    INTR_MUST_USE_RESULT T* Front() { return buffer_.Front(); }
    ...
    // operator.
    INTR_MUST_USE_RESULT std::optional<T> MoveFront() {
      if (Empty()) {
        return std::nullopt;
      }
    ...
    INTR_MUST_USE_RESULT std::optional<T> Pop();
  ...
  class Writer {
   public:
    ...
    INTR_MUST_USE_RESULT bool Insert(const T& item);
